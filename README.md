...
- **Протокол оценивания:** *one-step-ahead* на студентах из тестового сплита.
  ...
  последовательности, кроме первого. Это объясняет, почему для рекуррентных моделей `n = 44 581`, а для
...
После очистки и фильтрации получаются следующие характеристики
полного датасета:
...
Для быстрых проверок в репозиторий включён файл `data/sample/assistments_sample.csv` — подвыборка из первых
...
Microsoft Research). FLAML получает на вход табличные признаки, бюджет
...
Ниже представлены ключевые графики:
...
### 13.2 Дополнительная контейнерная доставка
...
Реализован автоматизированный ML-пайплайн на
