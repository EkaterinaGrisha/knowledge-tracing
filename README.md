...
**Дата:** май 2026
**Репозиторий:** `https://github.com/<логин>/Grigoreva-ES-ml-pipeline-knowledge-tracing` *(ссылка
будет проставлена после публикации; см. Приложение В).*
...
17. [Приложение В. Публикация в новый репозиторий](#приложение-в-публикация-в-новый-репозиторий)
18. [Приложение Г. Соответствие критериям оценивания](#приложение-г-соответствие-критериям-оценивания)
...
результатам, мониторингу, упаковке и воспроизводимости. Маппинг разделов на
критерии оценивания приведён в Приложении Г.
...
## Приложение В. Публикация в новый репозиторий

Каталог проекта самодостаточен и переносится одним архивом; пересоздание из
архива в новый репозиторий выполняется так:

### Шаг 1. Создать публичный репозиторий на GitHub

Имя должно содержать ФИО автора, например:
`Grigoreva-ES-ml-pipeline-knowledge-tracing`.
При создании *не* добавлять README, .gitignore и LICENSE — они уже есть в
каталоге проекта.

### Шаг 2. Запушить содержимое в новый репозиторий
...
git init
git add .
git commit -m "Automated ML pipeline: knowledge tracing on ASSISTments 2009"
git branch -M main
git remote add origin https://github.com/<user>/Grigoreva-ES-ml-pipeline-knowledge-tracing.git
git push -u origin main
```

### Шаг 3. Дождаться автоматического CI

После первого push GitHub Actions запустит workflow из
`.github/workflows/ci.yml`. Прохождение проверок отображается во вкладке
*Actions* нового репозитория. Ожидаемый результат — две зелёные галочки рядом
с коммитом: `lint-and-test` и `docker-smoke`.

### Шаг 4. Заменить плейсхолдер в README

В шапке настоящего README заменить плейсхолдер
`https://github.com/<логин>/...` на фактический адрес репозитория и
зафиксировать изменение отдельным коммитом.

### Шаг 5. (Опционально) Защитить ветку main

Settings → Branches → Add rule → require status checks — это запретит push
в `main`, если CI не зелёный.

---

## Приложение Г. Соответствие критериям оценивания

| Критерий | Баллы | Реализация | Раздел отчёта |
|---|---|---|---|
| Описание проекта: бизнес-задача, схема пайплайна, описание ETL, архитектуры ML-модели, метрик, визуализаций | 10 | настоящий README | §1–§3, §4, §5, §7, §9, §10 |
| AutoML / кастомная модель — автоматизированное обучение ML-модели | 20 | три уровня автоматизации: FLAML, Optuna для DKT, сквозной оркестратор `pipeline.py` | §7.3, §7.4, §7.5 |
| Тестирование на pytest | 5 | 7 файлов в `tests/`, маркер `slow` для тяжёлых тестов | §11 |
| Создание контейнера Docker: текст Dockerfile с описанием команд, функции контейнеризации | 15 | `Dockerfile` + `docker-compose.yml` + `.dockerignore`, разбор команд и функций безопасности / оптимизации / воспроизводимости | §12.1–§12.4 |
| CI/CD: описание непрерывной интеграции, список git-команд | 15 | `.github/workflows/ci.yml` (lint + pytest + docker build + smoke), список git-команд для доставки | §13.1–§13.2 |
| Мониторинг качества модели и инфраструктуры | 5 | три механизма: data-quality gate, дрейф (PSI + KS), ресурсы (psutil), всё в MLflow | §8.1–§8.4, скриншоты §9.4 |
| GitHub-репозиторий со ссылкой | 5 | публикация согласно Приложению В, ссылка в шапке отчёта | Приложение В |
| Презентация (5–7 слайдов) | 5 | `presentation/presentation.pptx`, генерируется командой `make present` или ячейкой ноутбука | модуль `src/presentation/build_deck.py` |
| **Итого** | **80** | | |
